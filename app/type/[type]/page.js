import {  fetchPokemonDetails, fetchPokemonListByCategorys } from '../../../utils/FetchPokemons';
...
export default async function CategorizedPage({ params }) {
  ...
  const types = ['normal','fighting','flying','poison','ground','rock','bug','ghost']; // Update this list based on available types

  // Fetch Pokémon based on the selected type
  try {
    const pokemonList = await fetchPokemonListByCategorys(type);
    ...
    const allPokemonSpecies = selectedTypeDetails.pokemon || [];
    const pokemonDetails = await Promise.all(
      allPokemonSpecies.map(pokemon =>
        fetchPokemonDetails(pokemon.pokemon.url.replace('pokemon-species', 'pokemon'))
      )
    );
    ...
    return (
      <section className="py-20 h-auto bg-gradient-to-r from-green-500 to-blue-500 text-white">
        <div className="container mx-auto text-center">
          ...
          <TypeDropdown types={types} str = "type" />
